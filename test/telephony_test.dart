 ...
 group('telephony', () {
   group("should request", () {
     test("sms permissions", () async {
       ...
       verify(methodChannel.invokeMethod<bool>(REQUEST_SMS_PERMISSION)).called(1);
     ...
     test("phone permissions", () async {
       ...
       verify(methodChannel.invokeMethod<bool>(REQUEST_PHONE_PERMISSION)).called(1);
       expect(permissionGranted, true);
     });
     ...
     test("phone and sms permissions", () async {
       ...
       verify(
           methodChannel.invokeMethod<bool>(REQUEST_PHONE_AND_SMS_PERMISSION)).called(1);
       expect(permissionGranted, true);
     });
   });
   ...
   group("should send", () {
     ...
     test("sms by default app", () async {
       ...
       verify(methodChannel.invokeMethod(
           SEND_SMS_INTENT, {"address": "123456", "message_body": "message"})).called(1);
     });
   });
   ...
   group("should query", () {
     ...
     test("inbox with filters", () async {
       ...
       verify(await methodChannel.invokeMethod(GET_ALL_INBOX_SMS, args)).called(1);
     ...
     test("sent with filters", () async {
       ...
       final sent = await telephony.getSentSms(
           columns: columns, filter: filter, sortOrder: sortOrder);
       verify(await methodChannel.invokeMethod(GET_ALL_SENT_SMS, args)).called(1);
       expect(
     ...
     test("draft with filters", () async {
       ...
       final drafts = await telephony.getDraftSms(
           columns: columns, filter: filter, sortOrder: sortOrder);
       verify(await methodChannel.invokeMethod(GET_ALL_DRAFT_SMS, args)).called(1);
     ...
     test("conversations", () async {
       ...
       verify(methodChannel.invokeMethod(GET_ALL_CONVERSATIONS, args)).called(1);
       expect(
           conversations[0]
     ...
     test("conversations with filter", () async {
       ...
       verify(await methodChannel.invokeMethod(GET_ALL_CONVERSATIONS, args)).called(1);
       expect(
           conversations[0]
               .equals(SmsConversation.fromMap(mockConversations[0])),
           isTrue);
   ...
   group("should initiate call", () {
     test("via default phone app", () async {
       ...
       when(methodChannel.invokeMethod(OPEN_DIALER, args)).thenAnswer((realInvocation) async {});
     ...
     test('directly', () async {
       ...
       when(methodChannel.invokeMethod(DIAL_PHONE_NUMBER, args)).thenAnswer((realInvocation) async {});
